...
# Hard coded file path due to issues with directing to proper folder
...
# final_path_to_ckpt = os.path.join(path_to_learn_pkg, 'learned-model/frozen_interference_graph.pb')
final_path_to_ckpt = os.path.join(path_to_learn_pkg, 'learned-model/frozen_inference_graph.pb')
...
# Hard coded file path due to issues with csv file redirection
...
with detection_graph.as_default():
  ...
  with tf.io.gfile.GFile(final_path_to_ckpt, 'rb') as fid:
...
# print(numberof_validation_img);

TEST_IMAGE_PATHS = [ os.path.join(final_path_to_test_img, 'image{}.jpg'.format(i)) for i in range(1, numberof_validation_img+1) ]

# TEST_IMAGE_PATHS = [ os.path.join(final_path_to_test_img, 'image{}.jpg'.format(i)) for i in range(1, 2) ]
